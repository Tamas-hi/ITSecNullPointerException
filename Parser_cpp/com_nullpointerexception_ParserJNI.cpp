...
CaffHeader subvectorToHeader(const std::vector<unsigned char>& subvector) {
    ...
    std::vector<unsigned char> size_vector(&subvector[4], &subvector[4 + sizeof(long)]);
    header_size = subvectorToLong(size_vector);
    
    std::vector<unsigned char> num_vector(&subvector[4 + sizeof(long)], &subvector[subvector.size()]);
...
CaffCredits subvectorToCredits(const std::vector<unsigned char>& subvector) {
    ...
    std::vector<unsigned char> len_vector(&subvector[6], &subvector[6 + sizeof(long)]);
    creator_len = subvectorToLong(len_vector);
    
    CaffCredits caffCredits = CaffCredits(creator_len);
    
    std::vector<unsigned char> creator_vector(&subvector[6 + sizeof(long)], &subvector[subvector.size()]);
...
CaffAnimation subvectorToAnimation(const std::vector<unsigned char>& subvector) {
    ...
    std::vector<unsigned char> duration_vector(&subvector[0], &subvector[sizeof(long)]);
...
CiffHeader subvectorToCiffHeader(const std::vector<unsigned char>& subvector) {
    ...
    std::vector<unsigned char> header_size_vector(&subvector[_current_location], &subvector[_current_location + sizeof(long)]);
    header_size = subvectorToLong(header_size_vector);
    
    _current_location = _current_location + sizeof(long);
    
    std::vector<unsigned char> content_size_vector(&subvector[_current_location], &subvector[_current_location + sizeof(long)]);
    content_size = subvectorToLong(content_size_vector);
    
    _current_location = _current_location + sizeof(long);
    
    std::vector<unsigned char> width_vector(&subvector[_current_location], &subvector[_current_location + sizeof(long)]);
    width = subvectorToLong(width_vector);
    
    _current_location = _current_location + sizeof(long);
    
    std::vector<unsigned char> height_vector(&subvector[_current_location], &subvector[_current_location + sizeof(long)]);
...
CaffReturnData readBlocks(const vector<unsigned char>& v)
{
    ...
    while(i < v.size()) {
        ...
        std::vector<unsigned char> sub(&v[block_beg + 1], &v[block_beg + sizeof(long) + 1]);
        block_length = subvectorToLong(sub);
        i = block_beg + sizeof(char) + sizeof(long);
        if(block_id == '\x01') {
            ...
            cout << "   *** CIFF HEADER *** \n";
            std::vector<unsigned char> ciff_data(&v[i + sizeof(long)], &v[i + block_length]);
            ...
            std::vector<unsigned char> ciff_content_vector(&v[i + sizeof(long) + ciff_header.getHeader_size()], &v[i + block_length]);
        ...
        block_beg = sizeof(char) + sizeof(long) + block_length + block_beg;
        i = block_beg;
    }
...
JNIEXPORT jobject JNICALL Java_com_nullpointerexception_ParserJNI_readData(JNIEnv * env, jobject, jbyteArray array) {
    ...
    // caption
    long string_length = return_data.getCaption().length();
